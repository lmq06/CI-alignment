...
# select shared sample between rna data and survival data
...
# patients are divided into 2 group
USP20.high <- as.numeric(rna.data["USP20", ] > median(rna.data["USP20", ]))

# construct data1 in order to use ggsurvplot
...
# find shared sample between meth.data, rna.data and surv.data
...
# Classification by USP20 expression level
USP20.high2 <- as.numeric(as.numeric(rna.data2["USP20", ]) > median(as.numeric(rna.data2["USP20", ])))

# Confirm whether the data obey a normal distribution, only 6 probes, so i maunall
shapiro.results <- array(NA, c(nrow(meth.data.USP20), 1))
for (i in 1:nrow(meth.data.USP20))
{
  shapiro.results[i,1] <- shapiro.test(as.numeric(meth.data.USP20[i, ]))$p.value
}

# all data obey normal distribution, usinf default method
for (i in 1:nrow(meth.data.USP20))
{
...
length(which(cnv.USP20 < 0))
length(which(cnv.USP20 > 0))
length(which(cnv.USP20 == 0))

# Prognostic biomarker related differentially expressed genes

# rna.data and USP20.high is previously processed
group <- cbind(USP20.high, 1 - USP20.high)
